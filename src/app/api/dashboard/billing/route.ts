...
export async function GET(request: NextRequest) {
  try {
    ...
    // Mock billing data based on actual business plan
    ...
    const currentPlan = primaryBusiness ? getPlanDetails(primaryBusiness.planType) : getPlanDetails('FREE_TRIAL');

    // Calculate next billing date (30 days from now for active subscriptions)
    const nextBillDate = primaryBusiness?.planType !== 'FREE_TRIAL' 
    ...
    const invoices = primaryBusiness?.planType !== 'FREE_TRIAL' ? [
      {
        id: 'inv_001',
    ...
    const usage = {
      ...
      maxBusinessListings: primaryBusiness?.planType === 'VIP' ? 999 : primaryBusiness?.planType === 'PRO' ? 5 : 1,
    ...
    const billingData = {
      currentPlan: {
        ...
        trialEndsAt: primaryBusiness?.trialEndsAt?.toISOString().split('T')[0] || null,
        nextBillDate,
      },
      paymentMethod,
      invoices,
